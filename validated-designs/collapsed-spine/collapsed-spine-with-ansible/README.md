...
- **List-keys come from the released path catalog, not guesswork.**
  `tools/audit-srl-paths` syncs `ansible/srl_list_keys.py` from SR Linux's
  `paths.json`, so the renderer keys YANG lists by full path context (the same node
...
ansible/
  ...
  filter_plugins/srl_payload.py   # readable srl_config tree -> SR Linux operations
  srl_list_keys.py                # YANG list-key map (generated by audit-srl-paths)
  roles/
    srl_config/                   # validate + apply fabric intent, one transaction
    srl_validate/                 # assert SR Linux state
    linux_endpoint/               # configure the traffic-source containers
playbooks/
  deploy.yml                      # apply intent
  validate.yml                    # assert state (BGP, ES, LAG, pings)
tools/
  audit-srl-paths                 # sync list-keys from the SR Linux path catalog
...
Tested with SR Linux `25.3.2`, `ansible-core` `2.21.0`, `nokia.srlinux` `1.1.0`.
...
| --- | --- | --- | --- |
| `set:` | `update` | Merge — add/modify only the leaves you list; the rest is left alone | `state: merged` |
...
- Use **`set:`** for normal additive intent — safe, non-destructive.
...
This design uses `set:` + targeted `delete:` by choice: non-destructive on shared
...
- `{}` means an intentional empty payload at that exact path (presence/empty in
  `set:`/`replace:`; the point to prune in `delete:`).
- Shorthand like `encap: untagged`, `primary: true`, `activation-timer`, and
  `interface-standby-signaling-on-non-df` is renderer behavior in `srl_payload.py`,
  not generic YANG.
...
```
```yaml
srl_config: {set: {interface: {ethernet-1/51: {description: tor2-spine-lag}}}}
...
```
```yaml
srl_config: {set: {network-instance: {v10-simple: {interface: {lag1.4096: {}}}}}}
...
`srl_payload.py` keys YANG lists by full path context using
`ansible/srl_list_keys.py`. Keep that map honest with `tools/audit-srl-paths`, which
fetches a release `paths.json`, extracts `type=[list]` entries, and compares the
discovered keys with `LIST_KEY_PATHS`:

```bash
tools/audit-srl-paths --version v25.3.2                 # check against a release
...
- **`tools/audit-srl-paths`** — syncs list-key rendering against a given SR Linux
